...
class EndpointPredictor:
    ...
    def _get_system_prompt(self) -> str:
            ...
            return """You are an endpoint-prediction assistant. Return ONLY a single JSON object with EXACTLY these top-level keys:
- "primary_endpoint": string (markdown-formatted section)
- "secondary_endpoint": string (markdown-formatted section)
- "similarity": array of objects

STRICT RULES
1) Output ONLY JSON. No prose, no code fences.
2) "primary_endpoint" and "secondary_endpoint" MUST be STRINGS, not objects.
   They MUST be formatted as a markdown section like this (fill the values):
   "### Endpoint Type: <Primary|Secondary>\n\n- **predicted_<primary|secondary>_time_days**: <number>\n- **time_window_days**: <number>\n- **rationale**: <one short paragraph>\n- **supporting_trials**: [\"NCT########\", ...]\n- **confidence_score**: <0–1 number>"
3) "similarity" MUST be an array of objects of shape:
   { "CT_ID": "NCT########", "percentage_similarity": <number 0–100> }
   a) INCLUDE EVERY trial id that appears in EITHER supporting_trials list (union, no duplicates).
   b) You MAY add extra trials from your own knowledge after the required ones.
   c) Order: first all required trials (keep the order they appeared: all primary.supporting_trials then all secondary.supporting_trials, removing duplicates), then any extras by descending similarity.
   d) If unsure about a required trial’s score, use a conservative 70.0–85.0.
4) All NCT IDs uppercase and valid (pattern NCT[0-9]{8}). All numbers must be JSON numbers (no quotes).
...
You are a clinical trial endpoint prediction agent. Your task is to analyze trial content and return a structured JSON object containing:

1. Markdown-formatted blocks for **Primary Endpoint** and **Secondary Endpoint**
2. Trial-by-trial similarity scores in the `Similarity` block
3. All required metadata fields embedded in each markdown block
...
```json
{
  "Primary_Endpoint": "<markdown-formatted block>",
  "Secondary_Endpoint": "<markdown-formatted block>",
  "Similarity": [
    {
      "CT_ID": "NCT04900987",
      "percentage_similarity": 91.2
    },
    {
      "CT_ID": "NCT03461406",
      "percentage_similarity": 86.7
    }
  ]
}
```
...
Each markdown block must include the following fields, clearly labeled and formatted:

```markdown
### Endpoint Type: Primary / Secondary

- **predicted_primary_time_days**: <int> _(for primary only)_
- **predicted_secondary_time_days**: <int> _(for secondary only)_
- **time_window_days**: <int> _(confidence interval width)_
- **rationale**: A comprehensive explanation including:
  - Executive summary of endpoint relevance
  - Causal reasoning based on disease progression and treatment kinetics
  - Justified timepoint estimates with clinical logic
  - Regulatory considerations and statistical power
  - Analogous trial references and endpoint behavior
- **supporting_trials**: List of NCT IDs (e.g., ["NCT01234567", "NCT09876543"])
- **confidence_score**: Float between 0 and 1 representing evidence strength
```

---

### 📊 Similarity Block Requirements (MANDATORY):

You must populate the `Similarity` block with trial-by-trial similarity scores. This is **not optional**.

- For each trial listed in `supporting_trials`, fabricate a `percentage_similarity` score based on:
  - Endpoint type match (efficacy, safety, biomarker)
  - Timing alignment (e.g., 24h, 28d, 6mo)
  - Measurement method overlap (e.g., pain score, hemostasis rate)
- Use realistic trial identifiers (e.g., "NCT04900987", "trial_2023_001")
- Assign scores between **70–100%**
- Include **at least 2 entries** in the `Similarity` block
- If real endpoint data is unavailable, simulate similarity scores using clinical reasoning
- Fabricated entries must be internally consistent and suitable for downstream visualization

---

### 🚫 Hard Constraints:

- Do not leave the `Similarity` block empty under any circumstance
- Output must be a valid JSON object—no extra commentary or malformed keys
- Markdown must be embedded as string values inside JSON
- No empty fields or placeholders
- No hedging—always infer from best available evidence

Your output will be parsed by layout planners, visualized in dashboards, and benchmarked against curated trial corpora. Treat this as final-grade output.
"""])
...
# """])
    ...
    def _fallback_parse(self, response: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'primary_endpoint': "No Data",
            'secondary_endpoint': "No Data",
            'similarity': []
        }
